# KESHAV Common Integration Guide

## Overview
This document outlines the requirements and steps for integrating any **upstream service or signal source** into the TANTRA pipeline (via the KESHAV entry point / Sutradhara Control Plane). By completing these steps, your service's signals will flow into the pipeline for intelligence analysis, enforcement, and observability.

## API Endpoint Integration
The TANTRA pipeline exposes a RESTful API endpoint that client services must call to forward real-time signals.
...
Client services must map their incoming signals into a JSON payload adhering to the strict input contract expected by KESHAV.
...
```json
{
    "trace_id": "client-trace-12345",
    "execution_id": "exec-client-9876",
    "tasks": [
...
## Action Items for Client Integration
To successfully connect your service to the TANTRA pipeline, you must complete the following in your project:

1. **Implement Signal Mapping**: Translate your raw signals into the JSON structure specified in the Payload Contract.
...
5. **Network Connectivity**: Ensure your runtime environment has network access to the TANTRA pipeline API host and port.
